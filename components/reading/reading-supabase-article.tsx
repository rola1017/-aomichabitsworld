...
function absolutizeOgImage(url: string | undefined | null, siteOrigin: string): string {
...
export async function generateReadingArticleMetadata(
  category: string,
  rawId: string
): Promise<Metadata> {
  const wpId = Number(decodeURIComponent(rawId))
  if (!Number.isFinite(wpId) || wpId <= 0) return { title: "文章" }
  const post = await getPostByWpId(wpId)
  if (!post) return { title: "文章" }

  const siteOrigin = await getSiteOrigin()
  const canonical = `${siteOrigin}/reading/${category}/${wpId}`
...
type ReadingArticleProps = { wpId: number }

export async function ReadingSupabaseArticle({ wpId }: ReadingArticleProps) {
  const post = await getPostByWpId(wpId)
  if (!post) notFound()

  return (
    <div className="rounded-2xl border border-[#D1C7B7] bg-white/80 p-6 shadow-md sm:p-8">
      <h1 className="mb-3 text-2xl font-bold text-[#3d3630]">{post.title}</h1>
      <div
